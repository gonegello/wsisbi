     ...
     <!--Card header-->
      ...
      <div class="card-body" style="border-radius:5px;border:1px solid #ededed;border-bottom:none;
        ...
        <div class="row">
          ...
          <div class="col">
            <span>Date of Request: <?php echo $r_date;?></span>
       
          </div>

        <div class="col">
        <p style="float:right;color:grey;">To : <a href="#"><?php echo $dept_head;?></a> <span><i>( Department Head )</i></span><br>
        <span>Requested By:<a href="#"> <?php echo $getname;?></a></span></p>
        </div>
        ...
        <form method="post" action = "pr-h-accept.php">
        <table>
        ...
        where stocks.int_typeid = $ics and cart.stat = '{$stat}' and cart.client_id = $client_id and pr_no = $pr_no");
        while($row = mysqli_fetch_array($query))
        {
      ...
      ?>
        ...
        <tr>
            <td>
              ...
              <input type="hidden" name="hd_aprvd[]" value="<?php echo $datte;?>"/>
    ...
    <!--    <button type="submit" id="proceedics">Send Request</button>
      ...
      <div class="card-body" style="border-radius:5px;border:1px solid #ededed;border-bottom:none;
        ...
        <div class="row">
          ...
          <div class="col">
        <p style="color:grey;">Date of Request : <?php echo $r_date;?></p>
        </div>
      ...
      <!--  <form method="post" action="a-pr-ris.php">-->
        ...
        $stat = "1";
        $query = mysqli_query($conn, "select * from `cart` join stocks on stocks.s_id = cart.item_id
        where stocks.int_typeid = $ris and cart.stat = '{$stat}' and cart.client_id = $client_id and pr_no = $pr_no");
        while($row = mysqli_fetch_array($query))
        {
          $uprice = $row['unit_price'];
          $quantity = $row['quan'];
      ...
      ?>
        ...
        <tr>
            ...
            <input type="hidden" name="idcart2[]" value="<?php echo $row['idcart'];?>"/>
              <input type="hidden" name="hd_aprvd2[]" value="<?php echo $datte;?>"/>
              <input type="hidden" name ="stat2[]" value="2"/>

              <input type="hidden" name="time2[]" value="<?php echo $time;?>" />
              <input type="hidden" name="client_id2[]" value="<?php echo $row['client_id'];?>"/>
              <input type="hidden" name="act2[]" value="approved your purchase request"/>
              <input type="hidden" name="by2[]" value="<?php echo $id;?>"/>
            </td>
            <td><?php echo $row['unit'];?></td>
            <td><?php echo $row['brand']; echo " "; echo $row['stock_name'];?> <?php echo $row['description'];?></td>
            <td style = "text-align:right;"><?php echo $row['quan'];?></td>
    ...
    ?>
      ...
      <div class="card-body"  style="border-radius:5px;border:1px solid #eded;border-top:none;
        border-top-left-radius:0px;border-top-right-radius:0px;margin-left:5%;margin-right:5%;
         ...
         <div class="row">
          <div class="col">
        <button type="submit"  onclick = "return confirm('Are you sure you want to approve this request? Click `Ok` to Proceed.')" name="request" id="proceedris">Approve</button>
  </form>
        </div>
        </div>
      </div>
