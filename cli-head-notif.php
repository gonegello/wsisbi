...
<div class="container-fluid" style="background-color:whitesmoke;">
  <div class="card-shadow">
     ...
     <?php 
          ...
          if(isset($_SESSION['head_approve']))
             {
      ?>
              ...
              </div><hr><i><?=$_SESSION['head_approve'];?></i>
              </div>
             
      <?php 
          unset($_SESSION['head_approve']);
            }
      ?>
      ...
      <?php
        ...
        join clients on clients.id = cart.client_id where cart.stat = $stat and clients.office = '{$office}' and cart.h_id = $id group by pr_no");
      ...
      if($ris_ > 0 && $ics_ > 0)
      {
        echo '<a href="h-accept-pr.php?client_id='.$row['client_id'].'&r_date='.$row['r_date'].'&pr_no='.$row['pr_no'].'" id="link-a">';
      }
      else if($ris_ > 0 && $ics_ == 0)
      {
        echo '<a href="h-acc-ris.php?client_id='.$row['client_id'].'&r_date='.$row['r_date'].'&pr_no='.$row['pr_no'].'" id="link-a">';
      }
      else if($ics_ > 0 && $ris_ == 0)
      {
        echo '<a href="h-acc-ics.php?client_id='.$row['client_id'].'&r_date='.$row['r_date'].'&pr_no='.$row['pr_no'].'" id="link-a">';
        
      }

      ?>
     <!-- <a href="h-accept-pr.php?client_id=<?php //echo $row['client_id']?>&r_date=<?php //echo $row['r_date']?>" id="link-a"> -->
      ...
      border-bottom-right-radius:0px;margin-right:10%;margin-left:10%;">
        <div class="row">
          ...
          <div class="col">
            <span style="float:right;color:grey;"><?php echo $row['r_date'];?></span>
          </div>
        </div>
