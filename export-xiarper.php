...
while($row=mysqli_fetch_array($que))
{
	...
	$getdate = $row['date'];
	$getfc_id = $row['fc_id'];
    $getitemdetails = $row['item_details'];
    $getunitt = $row['unit'];
    $getinquann = $row['in_quan'];
    $getunitcostt = $row['unit_cost'];
    $gettotalcostt = $row['total_cost'];
...
$output = '';
if(isset($_POST["csv"])){
 $sql = "SELECT * FROM `xicspar` join fund_c on fund_c.fundc_id = xicspar.fc_id where id_iar = $iar_id";
...
<table class="table" bordered ="1">
	...
	<tr>
          ...
          ';
            ...
            $query = mysqli_query($conn, "select *from `xicspar` where id_iar = $iar_id");
            while($row = mysqli_fetch_array($query))
            {
